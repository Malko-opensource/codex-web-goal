...
async function main() {
  ...
  if (command === 'doctor') {
    ...
    console.log(JSON.stringify({ bridge: await client.call('web_goal_status'), nativeThreads: await client.call('web_goal_threads') }, null, 2)); return;
  }
  if (command === 'codex') {
  ...
  try {
    ...
    const bridge = new Bridge(store, new Workspace(workspaceRoot), native); await bridge.recover();
    ...
    let stopping = false;
    shutdown = async () => { if (stopping) return; stopping = true; bridge.close(); await servers.close(); child?.kill('SIGTERM'); await fs.unlink(lockPath).catch(() => {}); };
